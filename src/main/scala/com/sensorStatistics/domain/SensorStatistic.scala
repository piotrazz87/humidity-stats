...
final case class SensorFailureStatistic(sensorId: String) extends SensorStatistic {
  override def toString: String = s"$sensorId, NaN, NaN, NaN"
  override def avgHumidity: Option[Double] = None
}

final case class SensorSuccessStatistic(sensorId: String, min: Int, avg: Double, max: Int, qtyProcessed: Int)
